...
const Timer: React.FC<TimerProps> = ({ targetDate }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center p-8 relative">
      ...
      <h2
        className="text-2xl md:text-3xl font-light mb-8 uppercase tracking-widest text-center"
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-10">
        <TimeUnit value={timeLeft.days} label="Ngày" type="days" theme={theme} />
        <TimeUnit value={timeLeft.hours} label="Giờ" type="hours" theme={theme} />
        <TimeUnit value={timeLeft.minutes} label="Phút" type="minutes" theme={theme} />
        <TimeUnit value={timeLeft.seconds} label="Giây" type="seconds" theme={theme} />
      </div>

      <div className="mt-10 text-center">
        <p className="text-lg font-medium animate-pulse" style={{ color: theme.accentColor }}>
          {timeLeft.isPast
...
interface TimeUnitProps {
  ...
  theme: import('../ThemeContext').ThemeConfig;
}

const TimeUnit: React.FC<TimeUnitProps> = ({ value, label, type, theme }) => {
  ...
  useEffect(() => {
    ...
    if (value !== prevValueRef.current) {
      const isRoundNumber = value % 10 === 0 && value !== 0;
  ...
  const renderEasterEggIcon = (eggType: EggType) => {
    switch (eggType) {
      ...
      case 'love':
        return (
          <>
            <Heart className="w-12 h-12 fill-current drop-shadow-[0_0_15px]" style={{ color: theme.primaryColor }} />
            <div className="absolute inset-0 rounded-full blur-xl animate-pulse" style={{ backgroundColor: `${theme.primaryColor}50` }}></div>
          </>
        );
      case 'diamond':
        return (
      ...
      case 'magic':
        return (
          <>
            <Sparkles className="w-12 h-12 fill-current drop-shadow-[0_0_15px]" style={{ color: theme.accentColor }} />
            <div className="absolute inset-0 rounded-full blur-xl animate-pulse" style={{ backgroundColor: `${theme.accentColor}50` }}></div>
          </>
        );
      case 'energy':
        return (
  ...
  const isSpecial = value % 10 === 0 && value !== 0;
  ...
  return (
    <div className="flex flex-col items-center relative group">
        ...
        {particles.map(p => (
          ...
          >
            {p.eggType ? (
              ...
              <Heart
                className={`fill-current ${type === 'days' ? 'w-8 h-8' :
                    type === 'hours' ? 'w-6 h-6' :
                      type === 'minutes' ? 'w-5 h-5' :
                        'w-4 h-4'
                  }`}
      ...
      <div
        className={`relative backdrop-blur-sm rounded-2xl p-4 md:p-6 w-24 md:w-32 shadow-lg border border-white/50 transition-all duration-300 ${isSpecial ? 'scale-110' : ''}`}
        style={{
          backgroundColor: 'rgba(255,255,255,0.4)',
          ...(isSpecial ? { boxShadow: `0 0 20px ${theme.primaryColor}40`, borderColor: theme.primaryColor } : {})
        }}
      >
        <span
          className={`block text-4xl md:text-5xl font-bold text-center font-mono transition-colors duration-300`}
          style={isSpecial
            ? { color: '#CA8A04' }
            : {
              backgroundImage: `linear-gradient(to bottom right, ${theme.primaryColor}, ${theme.accentColor})`,
        ...
        <div
          className="absolute -bottom-2 left-1/2 transform -translate-x-1/2 w-3/4 h-1 opacity-50 rounded-full"
          style={{
      ...
      <span
        className="mt-3 text-sm md:text-base font-medium uppercase tracking-wider"
        style={{ color: theme.textColor }}
      >
        {label}
      </span>
